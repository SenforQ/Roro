...
class _VipPageState extends State<VipPage> {
  ...
  Future<void> _initIAP() async {
    try {
      ...
      if (kIds.isEmpty) {
        if (mounted) {
          _showToast('No subscription products configured');
        }
        return;
      }
      ...
      if (response.error != null) {
        debugPrint('IAP Error: ${response.error!.code}, ${response.error!.message}');
      ...
      if (purchase.status == PurchaseStatus.purchased ||
        ...
        if (purchase.status == PurchaseStatus.purchased ||
          ...
          if (validProductIds.contains(purchase.productID)) {
            try {
              VipSubscription matchedSubscription;
              if (_selectedSubscription != null && 
                  _selectedSubscription!.productId == purchase.productID) {
                matchedSubscription = _selectedSubscription!;
              } else {
                matchedSubscription = _subscriptions.firstWhere(
                  (sub) => sub.productId == purchase.productID,
                  orElse: () => _subscriptions.first,
                );
              }
              final durationDays = matchedSubscription.id == 'weekly' ? 7 : 30;
              ...
              children: [
                ...
                Expanded(
                  child: Stack(
                    children: [
                      Positioned(
                        top: 260 - MediaQuery.of(context).padding.top - 56,
                        left: 0,
                        right: 0,
                        bottom: 0,
                        child: ClipRect(
