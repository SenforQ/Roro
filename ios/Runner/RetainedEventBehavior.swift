...
class RetainedEventBehavior {
    //: static let shared = AppPermissionTool()
    static let shared = RetainedEventBehavior()

    /// 获取麦克风权限
    //: func requestMicPermission(authBlock: @escaping (_ auth: Bool, _ isFirst: Bool) -> Void) {
    func group(authBlock: @escaping (_ auth: Bool, _ isFirst: Bool) -> Void) {
    ...
    func pendingCenter(authBlock: @escaping (_ auth: Bool, _ isFirst: Bool) -> Void) {
        //: if #available(iOS 14, *) {
        if #available(iOS 14, *) {
    ...
    func sumry(authBlock: @escaping (_ auth: Bool, _ isFirst: Bool) -> Void) {
